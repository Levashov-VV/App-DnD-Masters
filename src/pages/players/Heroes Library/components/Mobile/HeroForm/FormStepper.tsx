...
export function FormStepper({ currentStep, onStepClick }: FormStepperProps) {
  return (
    <div>
      <nav className="grid grid-cols-2 gap-[2vw]">
        {FORM_STEPS.map((step) => {
          ...
          return (
            ...
            >
              <div className="relative left-[0.5vw] flex items-center gap-[1vw]">
                <div
                  className={`
                    w-[4vw] h-[4vw] rounded-full flex items-center justify-center text-[1vh] font-mono
                ...
                <div>
                  <div className="text-[1.4vh]">{step.title}</div>
                  <div className={`text-[1.2vh] ${isActive ? 'text-amber-100' : 'text-gray-400'}`}>
                    {step.description}
                  </div>
                </div>
              </div>
