...
export function SpellInfoModal({ isOpen, spellId, onClose }: SpellInfoModalProps) {
  ...
  return (
    ...
    >
      <div
        className={`relative w-[60vw] bg-stone-900 border-4 border-amber-600 rounded-2xl transition-all duration-300 transform overflow-hidden ${
      ...
      >
        ...
        >
          ...
          <button
            type="button"
            onClick={onClose}
            className="w-[1.4vw] h-[1.4vw] text-[2vh] font-bold text-stone-900 hover:text-stone-900 bg-amber-600 hover:bg-amber-500 rounded-4xl transition-all border-amber-500/50 hover:border-amber-400"
          >
            ✕
          </button>
        </div>
