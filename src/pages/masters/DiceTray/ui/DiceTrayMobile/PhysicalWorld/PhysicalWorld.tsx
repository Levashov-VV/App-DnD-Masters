...
export const PhysicsWorld = ({
  ...
  const wallY = cy + floorY + wallHeight / 2;
  const actualFloorY = cy + floorY;
  ...
  return (
    <div className="absolute inset-0 z-80 pointer-events-none w-screen h-screen">
      <Canvas
        shadows
        camera={{ position: [0, 10, 6], fov: 50, near: 0.1, far: 100 }}
      ...
      >
        ...
        <Physics gravity={[0, -15, 0]} timeStep={1 / 120} debug={debug}>
          ...
          {debug && <OrbitControls enabled enableZoom={true} />}
          {children}
        </Physics>
      </Canvas>
    </div>
