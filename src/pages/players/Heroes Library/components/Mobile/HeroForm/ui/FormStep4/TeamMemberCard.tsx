...
export function TeamMemberCard({ member, onEdit, onRemove }: TeamMemberCardProps) {
  ...
  return (
    <div
      style={{ margin: '0.5vw' }}
      className="relative w-[40vw] h-[18vh] border-2 border-amber-600 bg-stone-800 rounded-lg p-[0.5vw] flex flex-col items-center gap-[0.5vw] hover:border-amber-500 transition-colors group"
    >
      <div
      ...
      >
        {displayAvatar ? (
          <img src={displayAvatar} alt={member.name} className="w-full h-full object-contain" />
        ) : (
      ...
      <div className="relative bottom-[1vh] flex flex-col text-center w-full">
        <div>
          <h3 className="text-[2vh] font-bold text-amber-100 truncate">
            {member.name || 'Без имени'}
          </h3>
        </div>
        <div style={{ marginRight: '0.2vw' }} className="w-[98%] grid grid-cols-2 gap-[1vh]">
          <p className="text-[1.2vh] text-amber-100/80 truncate">Раса: {member.race}</p>
          <p className="text-[1.2vh] text-amber-100/80 truncate">Класс: {member.class}</p>
          <p className="text-[1.2vh] text-amber-100/80 truncate">
            Уровень: {''}
            <span className="text-[1.2vh]">{member.level}</span>
          </p>
          {member.subclass && (
            <p className="text-[1.2vh] text-amber-100/80 truncate">Подкласс: {member.subclass}</p>
          )}
        </div>
      </div>

      {/* Кнопки */}
      <div className="absolute top-[0.5vh] right-[0.5vw] flex gap-[1.2vw] transition-opacity">
        <button
          type="button"
          onClick={onEdit}
