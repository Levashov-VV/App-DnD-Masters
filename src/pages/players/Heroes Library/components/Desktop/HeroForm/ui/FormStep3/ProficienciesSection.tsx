...
export function ProficienciesSection({
  ...
  return (
    <div
      className="col-span-2 w-[28.3vw] border-2 border-amber-600 bg-stone-800 rounded-lg"
      style={{ padding: '0vh 0.3vw' }}
    >
      <h3 className="text-[2vh] text-center font-bold text-amber-100">Владение снаряжением</h3>

      {/* ОРУЖИЕ */}
      <div>
        <h4 className="text-[1.6vh] font-bold text-amber-100">Оружие</h4>
        <div className="grid grid-cols-4 gap-[0.5vw]">
          {WEAPON_TYPES.map((type) => (
            <button
              ...
              className={`rounded-lg text-[1.3vh] transition-all border-2 ${
      ...
      <div>
        ...
        <div className="flex gap-[0.5vw]">
          <input
            ...
            className="flex-1 bg-stone-900 border-2 border-amber-600 rounded-lg text-[1.4vh] text-amber-100 placeholder-amber-600/50 focus:outline-none focus:border-amber-400 transition-colors uppercase"
          />
          <button
            type="button"
            onClick={addTool}
            ...
            className="bg-amber-600 hover:bg-amber-500 text-stone-900 rounded-lg text-[1.4vh] font-bold uppercase transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            Добавить
          </button>
        </div>
        ...
        {toolProficiencies.length > 0 && (
          <div className="grid grid-cols-2 gap-[0.5vw] max-h-[12vh] overflow-y-auto overflow-x-hidden">
