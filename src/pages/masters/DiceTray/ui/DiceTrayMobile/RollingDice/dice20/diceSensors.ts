...
export const buildD20SensorPointsLocal = (mesh: THREE.Mesh, faces = 20): THREE.Vector3[] => {
  ...
  const k = Math.min(faces, normals.length);
  const { centers } = kmeansUnitVectors(normals, k, 12);
